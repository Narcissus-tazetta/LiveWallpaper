...
/// ディスプレイ固定(オーバーライド)画面のための専用プレイヤー管理。
/// スロットの生成/破棄、隣接ウォームキャッシュ、再生位置の記憶と復元、
/// 全面被覆時の再生/停止反映を担う。「どの画面をどの動画で固定するか」自体は
/// [[WallpaperModel+DisplayOverrides]] が持ち、このファイルはその実行部分。
@MainActor
extension WallpaperModel {
    ...
    func setDedicatedPlaybackContinuityEnabled(_ enabled: Bool) {
        ...
        UserDefaults.standard.set(enabled, forKey: PrefsKey.dedicatedPlaybackContinuityEnabled)
        if !enabled {
